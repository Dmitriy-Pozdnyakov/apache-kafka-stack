...
def build_message(raw_text: str) -> dict:
    return {
        "event": raw_text,
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: python scripts/python/producer_basic.py '<message>'")
        return 1
    ...
    payload = build_message(sys.argv[1])
    future = producer.send(TOPIC, value=payload)
    record = future.get(timeout=10)

    print(
        f"Sent to topic={record.topic} partition={record.partition} offset={record.offset}"
    )
    producer.flush()
    producer.close()
    return 0
